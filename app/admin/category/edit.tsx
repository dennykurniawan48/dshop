...
function Edit(props: {
  ...
  const addCategory = useFormik({
    initialValues: props.selectedCategory ?? {
      id: "",
      name: "",
      thumbnail: "",
    },
    ...
    onSubmit: async (values) => {
      ...
      fetch("/api/category", {
        ...
        body: JSON.stringify({ ...values, id: props.selectedCategory?.id }),
      })
        .then((res) => {
          if (res.ok) {
  ...
  return (
    ...
    >
      <form
        className="w-1/3 p-4 bg-white rounded-lg flex flex-col space-y-4"
      ...
      >
        ...
        <div className="flex flex-col space-y-2">
          ...
          />
          {addCategory.touched.name && addCategory.errors.name && (
        ...
        <div className="flex flex-col space-y-2">
          ...
          />
          {addCategory.touched.thumbnail && addCategory.errors.thumbnail && (
            <span className="text-xs text-red-500">
        ...
        <div className="flex w-full justify-around mt-14">
          ...
          <button className="bg-green-600 px-4 py-2 text-white" type="submit">
            Save
          </button>
        </div>
      </form>
